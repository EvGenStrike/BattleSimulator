...
internal class Zombie : ITroop
{
    ...
    public Zombie(TeamEnum team, Vector2 initialPosition, int width = 0, int height = 0)
    {
        Team = team;
        SetStats();
    ...
    {
        Health = 70 + random.Next(9) - 4;
        Damage = 20 + random.Next(5) - 2;
        MoveSpeed = 5 + random.Next(3) - 1;
        AttackSpeed = 1 + (float)((random.NextDouble() - 0.5) / 2);
        AttackDistance = 40;
        Cost = 100;
    }
    ...
    {
        return new Zombie(team, initialPosition, width, height);
    }
